...
const DChat = ({ selectedUser, currentUser }) => {
  ...
  useEffect(() => {
    const createOrFetchConversation = async () => {
      ...
      try {
        const conversationsRef = collection(db, "doctorconversations");
        const q = query(
          conversationsRef,
        ...
        if (conversation) {
          const messagesRef = collection(
            db,
            `doctorconversations/${conversation.id}/messages`
          );
  ...
  useEffect(() => {
    if (selectedUser && selectedUser.id) {
      const userDocRef = doc(db, "doctors", selectedUser.id);
  ...
  const sendMessage = async () => {
    ...
    try {
      const messagesRef = collection(
        db,
        `doctorconversations/${conversationId}/messages`
      );
